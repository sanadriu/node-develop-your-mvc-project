...
describe("user-crud-operations", () => {
  ...
  describe("1. Get users", () => {
    ...
    test("1.6. Reply with 'bad request' if the specified page is not a number", async () => {
      ...
      const res = await request.get("/users?page=foo").auth(token, { type: "bearer" });
    ...
    test("1.7. Reply with 'not found' if the specified page does not exist", async () => {
      ...
      const res = await request.get("/users?page=1000").auth(token, { type: "bearer" });
    ...
    test("1.8. Successful operation if specified page exists", async () => {
      ...
      const res = await request.get("/users?page=1").auth(token, { type: "bearer" });
  ...
  describe("2. Get single user", () => {
    ...
    test("2.1. Allow users with 'main-admin' role to make the request", async () => {
      ...
      const res = await request.get(`/users/${idUser}`).auth(token, { type: "bearer" });
    ...
    test("2.4. Do not allow users with 'customer' role to make the request if they are not themselves", async () => {
      const { _id: idUser } = await UserModel.findOne({ firstname: "Alice" }).lean().exec();
    ...
    test("2.7. Reply with 'bad request' if the id is invalid", async () => {
      ...
      const res = await request.get("/users/foo").auth(token, { type: "bearer" });
  ...
  describe("3. Create user", () => {
    ...
    test("3.1. Allow users with 'main-admin' role to make the request", async () => {
      ...
      const res = await request.post("/users").auth(token, { type: "bearer" }).send(body);
    ...
    test("3.3. Do not allow users with 'customer' role to make the request", async () => {
      ...
      const res = await request.get("/users").auth(token, { type: "bearer" }).send(body);
    ...
    test("3.6. Reply with 'email account is already used' if the given email is registered", async () => {
      const { uid: token } = await UserModel.findOne({ role: "main-admin" }).lean().exec();

      await UserModel.create({ uid: "2000", ...body });
      ...
      expect(res.status).toBe(400);
      expect(res.body).toHaveProperty("success", false);
      expect(res.body).toHaveProperty("message", "Email account is already used");
    });
  });
  ...
  describe("4. Update user", () => {
    ...
    test("4.7. Reply with 'bad request' if the id is invalid", async () => {
      ...
      const res = await request.patch("/users/foo").auth(token, { type: "bearer" }).send(body);
  ...
  describe("5. Delete user", () => {
    ...
    test("5.1. Allow users with 'main-admin' role to make the request", async () => {
      ...
      const res = await request.delete(`/users/${idUser}`).auth(token, { type: "bearer" });
    ...
    test("5.7. Reply with 'bad request' if the id is invalid", async () => {
      ...
      const res = await request.delete("/users/foo").auth(token, { type: "bearer" });
  ...
  describe("6. User sync", () => {
    ...
    test("6.1. Allow users that are authenticated + registered to make the request", async () => {
      const { uid: token } = await UserModel.findOne().lean().exec();

      const res = await request.get("/users/sync").auth(token, { type: "bearer" });
    ...
    test("6.2. Do not allow unauthenticated users to make the request", async () => {
      const res = await request.get("/users/sync");
    ...
    test("6.3. Do not allow unregistered users to make the request", async () => {
    ...
    test("6.4. Successful operation returns the corresponding user data", async () => {
      const {
        uid: token,
        firstname,
        lastname,
        email,
        role,
      } = await UserModel.findOne().lean().exec();
  ...
  describe("7. User register", () => {
    ...
    test("7.1. Allow users that are unauthenticated to make the request + 7.2. Successful operation only returns 'success' as true", async () => {
    ...
    test("7.3. Reply with 'email account is already used' if the given email is registered", async () => {
      await UserModel.create({ uid: "2000", ...body });
  ...
  describe("11. Get orders", () => {
    ...
    test("11.1. Allow users with 'main-admin' role to make the request", async () => {
      ...
      const res = await request.get(`/users/${idUser}/orders`).auth(token, { type: "bearer" });
    ...
    test("11.9. Reply with 'bad request' if the specified page is not a number", async () => {
      const { uid: token } = await UserModel.findOne({ role: "main-admin" }).lean().exec();
